...
// Nova's checkout wilaya/commune/shipping picker — same logic as the shared
// CheckoutShippingFields.vue (GET /wilayas + GET /communes?wilayaId=), but
// built on Nova's own NovaSelect primitive instead of @nuxt/ui's Select,
// since Nova renders zero @nuxt/ui components anywhere.
//
...
<template>
  <div class="space-y-5">
    <div class="grid grid-cols-2 gap-4">
      <div>
        <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-black/70">الولاية</label>
        <NovaSelect :model-value="props.form.wilayaId" :items="wilayaItems" placeholder="اختر الولاية" @update:model-value="(v) => (props.form.wilayaId = v)" />
      </div>
      <div>
        <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-black/70">البلدية</label>
        <NovaSelect v-model="selectedCommuneId" :items="communeItems" :disabled="!selectedWilaya" :placeholder="selectedWilaya ? 'اختر البلدية' : 'اختر الولاية أولاً'" />
      </div>
    </div>
