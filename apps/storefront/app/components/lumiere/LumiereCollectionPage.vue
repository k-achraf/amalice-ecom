...
// Lumiere collection — black masthead band + filter sidebar + grid. Prop
// contract mirrors Catalog with category-scoped data.
    ...
    <div class="border-b border-black bg-black text-white">
      <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-16 lg:px-8">
        <nav class="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/50">
          ...
          <Icon name="i-lucide-chevron-right" class="size-3.5" />
          <span class="text-white/80">{{ props.category.name }}</span>
        </nav>
        <h1 class="font-display text-4xl text-white sm:text-5xl lg:text-6xl">{{ props.category.name }}</h1>
        <p v-if="props.category.description" class="mt-3 max-w-2xl text-white/60">{{ props.category.description }}</p>
      </div>
    </div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div class="flex gap-8">
        <aside class="hidden w-64 shrink-0 lg:block">
          <div class="sticky top-24 space-y-6 rounded border border-black/10 bg-white p-5">
            <div>
              <h3 class="mb-3 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.14em] text-black/50">
                <Icon name="i-lucide-sliders-horizontal" class="size-3.5" /> Price range
              </h3>
              <div class="flex items-center gap-2">
                <LumiereInput :model-value="props.minPrice?.toString() ?? ''" placeholder="Min" @update:model-value="onMinInput" />
                <span class="text-black/30">–</span>
                <LumiereInput :model-value="props.maxPrice?.toString() ?? ''" placeholder="Max" @update:model-value="onMaxInput" />
              </div>
              <LumiereButton block size="sm" class="mt-3" @click="props.onApplyPrice">Apply filter</LumiereButton>
            </div>
            <div class="rounded bg-primary-50 p-4 text-sm">
              <p class="font-bold text-black">Cash on delivery</p>
              <p class="mt-1 text-black/60">Pay only when it arrives.</p>
            </div>
          </div>
        </aside>

        <div class="min-w-0 flex-1">
          <p class="mb-6 text-xs font-semibold uppercase tracking-[0.14em] text-black/45">{{ props.data?.items.length ?? 0 }} products in this collection</p>
          ...
          <EmptyState v-else-if="!props.data?.items.length" icon="i-lucide-package-search" title="No products in this collection yet" description="Check back soon, or browse the full catalog.">
            <LumiereButton to="/catalog" class="mt-4">Browse all products</LumiereButton>
          </EmptyState>
          <template v-else>
