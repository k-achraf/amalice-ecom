  ...
  <div class="bg-white">
    <div class="border-b border-black bg-black text-white">
      <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <h1 class="font-display text-4xl text-white sm:text-5xl">Checkout</h1>
      </div>
    </div>
          ...
          <div class="mb-10 flex items-center justify-center gap-3">
            <template v-for="(s, i) in props.stepItems" :key="s.value">
              <div class="flex items-center gap-2">
                <div
                  class="flex size-8 items-center justify-center rounded-full border text-sm font-bold"
                  :class="s.value === props.step ? 'border-black bg-black text-white' : 'border-black/20 bg-white text-black/40'"
                >{{ i + 1 }}</div>
                <span class="hidden text-xs font-semibold uppercase tracking-[0.1em] sm:inline" :class="s.value === props.step ? 'text-black' : 'text-black/40'">{{ s.title }}</span>
              </div>
              <div v-if="i < props.stepItems.length - 1" class="h-px w-8 bg-black/15" />
            </template>
          </div>

          <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div class="lg:col-span-2">
              <div class="rounded border border-black/10 bg-white p-6 sm:p-8">
                <!-- Step 1: address -->
                <div v-if="props.step === 'address'">
                  <h2 class="font-display mb-1 text-2xl text-black">Delivery details</h2>
                  <p class="mb-6 text-sm text-black/55">Where should we ship your order?</p>
                  <form class="space-y-5" @submit.prevent="submitAddress">
                    <div>
                      <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-black/60">Phone</label>
                      <LumiereInput v-model="props.form.phone" type="tel" placeholder="+15551234567" icon="i-lucide-phone" />
                      <p v-if="fieldErrors['phone']" class="mt-1 text-xs font-semibold text-primary-700">{{ fieldErrors['phone'] }}</p>
                    </div>
                    <div>
                      <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-black/60">Name (optional)</label>
                      <LumiereInput v-model="props.form.name" icon="i-lucide-user" />
                    </div>
                    <div>
                      <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-black/60">Address line 1</label>
                      <LumiereInput v-model="props.form.address.line1" icon="i-lucide-map-pin" />
                      <p v-if="fieldErrors['address.line1']" class="mt-1 text-xs font-semibold text-primary-700">{{ fieldErrors['address.line1'] }}</p>
                    </div>
                    <div>
                ...
                <div v-else-if="props.step === 'review'">
                  <h2 class="font-display mb-1 text-2xl text-black">Review your order</h2>
                  <p class="mb-6 text-sm text-black/55">Everything look right?</p>

                  <div class="space-y-3 rounded border border-black/10 p-4">
                    <h3 class="text-xs font-bold uppercase tracking-wide text-black">Items</h3>
                    <ul class="divide-y divide-black/10">
                      <li v-for="item in props.cart.items" :key="item.productId" class="flex justify-between py-2 text-sm">
                        <span class="text-black/65">{{ item.name }} × {{ item.quantity }}</span>
                        <PriceDisplay :amount-cents="item.priceCents * item.quantity" class="font-bold" />
                      </li>
                    </ul>
                  </div>

                  <div class="mt-4 space-y-1 rounded border border-black/10 p-4 text-sm">
              ...
              <div class="sticky top-24 rounded border border-black bg-white p-6">
                ...
                <ul class="mb-4 max-h-64 space-y-3 overflow-y-auto pr-1">
                  <li v-for="item in props.cart.items" :key="item.productId" class="flex items-center gap-3">
                    <div class="size-12 shrink-0 overflow-hidden bg-neutral-100">
                      <NuxtImg v-if="item.imageUrl" :src="item.imageUrl" :alt="item.name" class="size-full object-cover" width="48" height="48" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <p class="line-clamp-1 text-sm font-medium text-black">{{ item.name }}</p>
                      <p class="text-xs text-black/45">Qty {{ item.quantity }}</p>
                    </div>
