...
// Trove's select — a real native <select>, styled (appearance-none + custom
...
<template>
  <div class="relative">
    <select
      :value="modelValue ?? ''"
      :disabled="disabled"
      class="w-full appearance-none rounded border border-neutral-200 bg-white px-4 py-2.5 pr-10 text-[var(--color-trove-ink)] transition-all duration-200 focus:border-primary-500 focus:shadow-[var(--shadow-trove-sm)] focus:outline-none disabled:cursor-not-allowed disabled:bg-neutral-100 disabled:opacity-60"
