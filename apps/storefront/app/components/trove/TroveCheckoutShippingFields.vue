...
// Trove checkout flow's wilaya/commune/shipping picker — mirrors
// CheckoutShippingFields.vue's cascade logic (GET /wilayas + GET
// /communes?wilayaId=) but built on Trove's own primitives (TroveSelect)
// instead of @nuxt/ui, since Trove renders zero @nuxt/ui components
// anywhere.
//
...
<template>
  <div class="space-y-4">
    <div class="grid grid-cols-2 gap-4">
      <div>
        <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-neutral-500">الولاية</label>
        <TroveSelect
      ...
      <div>
        <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-neutral-500">البلدية</label>
        <TroveSelect
    ...
    <div v-if="selectedWilaya" class="space-y-1.5">
      <label class="block text-xs font-bold uppercase tracking-wide text-neutral-500">طريقة التوصيل</label>
