...
// Trove's lead-form field renderer — same logic as the shared
// LeadFormFields (rows grouping, wilaya→commune cascade backed by GET
// /wilayas + GET /communes?wilayaId=), but built on Trove's own primitives
    ...
    <template v-for="(row, ri) in rows" :key="ri">
      <div :class="row.length === 2 ? 'grid grid-cols-2 gap-3' : ''">
        <div v-for="field in row" :key="field.id" class="space-y-1.5">
          <label class="block text-xs font-bold uppercase tracking-wide text-neutral-500">{{ leadFieldLabel(field) }}</label>
          <TroveSelect
