...
// Boutique's OTP pin input — plain <input> boxes + Tailwind, no @nuxt/ui.
// Sharp-cornered bordered boxes (a special isolated widget, so it gets a
// full 1px border rather than the underline-only field convention).
...
<template>
  <div class="flex justify-center gap-2" @paste="onPaste">
    <input
      ...
      maxlength="1"
      class="size-11 border border-default bg-transparent text-center text-lg font-normal tabular-nums text-highlighted focus:border-[var(--color-boutique-accent)] focus:outline-none sm:size-12"
