...
// Pulse PDP — white gallery + sticky glow-card order summary. Variants as
// rounded pill chips. Lead-form mode uses PulseLeadFormFields (cascading
...
<template>
  <div v-if="props.product" class="bg-white">
    ...
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <img v-if="props.landingPageImageUrl" :src="props.landingPageImageUrl" :alt="props.product.name" class="mb-10 w-full rounded-xl" />

      <div class="grid grid-cols-1 gap-10" :class="props.landingPageImageUrl ? '' : 'lg:grid-cols-5 lg:gap-8'">
        <!-- Gallery — floating glossy card, offset up and slightly narrower than half, echoing the hero's asymmetric device card -->
        <div v-if="!props.landingPageImageUrl" class="relative space-y-4 lg:col-span-2">
          <div class="glossy-highlight glow-card relative -rotate-1 aspect-square overflow-hidden bg-neutral-50 p-4 lg:mt-6">
            <NuxtImg
          ...
          <div v-if="props.galleryImages.length > 1" class="flex flex-wrap gap-3">
            <button
              v-for="(img, i) in props.galleryImages"
              :key="i"
              class="size-20 overflow-hidden rounded-2xl bg-neutral-50 ring-2 transition-all"
        ...
        <!-- Detail + order summary — the wider lane, so the split reads asymmetric rather than 50/50 -->
        <div class="space-y-6" :class="props.landingPageImageUrl ? '' : 'lg:col-span-3'">
          <div>
            ...
            <h1 class="font-display mt-1 text-4xl text-neutral-900 sm:text-5xl">{{ props.product.name }}</h1>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <PulseBadge v-if="!props.inStock" color="error">Out of stock</PulseBadge>
            <PulseBadge v-else-if="props.effectiveStock <= props.product.lowStockThreshold" color="primary">Only {{ props.effectiveStock }} left</PulseBadge>
          </div>
          ...
          <div v-for="(values, key) in props.variantOptions" :key="key" class="space-y-3">
            <p class="text-sm font-medium capitalize text-neutral-900">{{ key }}</p>
          ...
          <div v-if="props.displayCart !== false" class="glow-card p-6">
            <div class="mb-4 flex items-center gap-2">
              ...
              <h2 class="font-display text-xl text-neutral-900">Order summary</h2>
            </div>
            <div class="mb-4">
              <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-neutral-400">Quantity</p>
              <PulseQuantityStepper :model-value="props.quantity" :min="1" :max="props.effectiveStock" :disabled="!props.inStock" @update:model-value="props.onUpdateQuantity" />
            </div>
            ...
            <PulseButton :disabled="!props.inStock" icon="i-lucide-shopping-bag" size="lg" block class="mt-5" @click="props.onAddToCart">{{ props.inStock ? 'Add to cart' : 'Out of stock' }}</PulseButton>
          ...
          <div v-if="props.displayCart === false && props.inStock" class="glow-card space-y-4 p-6">
            <div class="flex items-center gap-2">
              ...
              <h2 class="font-display text-xl text-neutral-900">Order now — cash on delivery</h2>
            </div>
            <PulseLeadFormFields :fields="props.leadFields ?? []" :data="props.leadFormData ?? {}" />
            ...
            <div class="flex items-center gap-3">
              <PulseQuantityStepper :model-value="props.quantity" :min="1" :max="props.effectiveStock" @update:model-value="props.onUpdateQuantity" />
              <PulseButton block size="lg" :loading="props.leadPlacing" @click="props.onSubmitLead">Place order</PulseButton>
            </div>
    ...
    <section v-if="props.reviewData" class="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
      <div class="glow-card p-8">
        <h2 class="font-display mb-6 text-3xl text-neutral-900">Customer reviews</h2>
        <div v-if="props.reviewData.summary.count > 0" class="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div class="space-y-2">
            <span class="text-4xl font-semibold text-neutral-900">{{ props.reviewData.summary.average?.toFixed(1) }}</span>
            <div class="flex">
              <Icon
                v-for="i in 5"
                :key="i"
    ...
    <section v-if="props.product.related?.length" class="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
      <h2 class="font-display mb-6 text-3xl text-neutral-900">You may also like</h2>
