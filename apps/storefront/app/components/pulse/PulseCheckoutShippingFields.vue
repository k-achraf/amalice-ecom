...
// Cart+checkout flow's wilaya/commune/shipping picker for Pulse — mirrors
// CheckoutShippingFields.vue's cascade logic (GET /wilayas + GET
// /communes?wilayaId=) but built on Pulse's own primitives (PulseSelect)
...
<template>
  <div class="space-y-4">
    <div class="grid grid-cols-2 gap-4">
      <div>
        <label class="mb-1.5 block text-xs font-medium uppercase tracking-wide text-neutral-500">الولاية</label>
        <PulseSelect :model-value="props.form.wilayaId" :items="wilayaItems" placeholder="اختر الولاية" @update:model-value="(v) => (props.form.wilayaId = v)" />
      </div>
      <div>
        <label class="mb-1.5 block text-xs font-medium uppercase tracking-wide text-neutral-500">البلدية</label>
        <PulseSelect v-model="selectedCommuneId" :items="communeItems" :disabled="!selectedWilaya" :placeholder="selectedWilaya ? 'اختر البلدية' : 'اختر الولاية أولاً'" />
      </div>
    </div>

    <div v-if="selectedWilaya" class="space-y-1.5">
      <label class="block text-xs font-medium uppercase tracking-wide text-neutral-500">طريقة التوصيل</label>
