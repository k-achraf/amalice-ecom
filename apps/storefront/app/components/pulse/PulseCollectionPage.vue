  ...
  <div v-if="props.category">
    <div class="mesh-bg border-b border-neutral-100">
      <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <h1 class="font-display text-4xl text-neutral-900 sm:text-5xl">{{ props.category.name }}</h1>
        <p v-if="props.category.description" class="mt-3 max-w-2xl text-neutral-500">{{ props.category.description }}</p>
      </div>
    </div>
      ...
      <div class="flex gap-8">
          ...
          <div class="glow-card sticky top-24 space-y-6 p-5">
            <div>
              ...
              <div class="flex items-center gap-2">
                <PulseInput :model-value="props.minPrice?.toString() ?? ''" placeholder="Min" @update:model-value="onMinInput" />
                <span class="text-neutral-300">–</span>
                <PulseInput :model-value="props.maxPrice?.toString() ?? ''" placeholder="Max" @update:model-value="onMaxInput" />
              </div>
              <PulseButton block size="sm" class="mt-3" @click="props.onApplyPrice">Apply filter</PulseButton>
            </div>
        ...
        <div class="min-w-0 flex-1">
          ...
          <EmptyState v-else-if="!props.data?.items.length" icon="i-lucide-search-x" title="No products in this collection yet" description="Check back soon, or browse the full catalog.">
            <PulseButton to="/catalog" class="mt-4">Browse all products</PulseButton>
          </EmptyState>
          <template v-else>
