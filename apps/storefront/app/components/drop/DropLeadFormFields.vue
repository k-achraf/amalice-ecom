...
// Drop's lead-form field renderer — same logic as the shared LeadFormFields
// (rows grouping, wilaya→commune cascade backed by GET /wilayas + GET
// /communes?wilayaId=), but built on Drop's own primitives instead of
// @nuxt/ui, since Drop renders zero @nuxt/ui components anywhere.
    ...
    <template v-for="(row, ri) in rows" :key="ri">
      <div :class="row.length === 2 ? 'grid grid-cols-2 gap-3' : ''">
        <div v-for="field in row" :key="field.id" class="space-y-1.5">
          <label class="block text-xs font-bold uppercase tracking-wide text-white/50">{{ leadFieldLabel(field) }}</label>
          <DropSelect
