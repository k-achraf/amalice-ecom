...
// Drop checkout flow's wilaya/commune/shipping picker — same cascade logic as
// the shared CheckoutShippingFields.vue (GET /wilayas + GET
// /communes?wilayaId=), but built on Drop's own primitives (DropSelect)
// instead of @nuxt/ui, since Drop renders zero @nuxt/ui components anywhere.
//
  ...
  <div class="space-y-4">
    <div class="grid grid-cols-2 gap-3">
      <div class="space-y-1.5">
        <label class="block text-xs font-bold uppercase tracking-wide text-white/50">الولاية</label>
        <DropSelect v-model="props.form.wilayaId" :items="wilayaItems" placeholder="اختر الولاية" />
      </div>
      <div class="space-y-1.5">
        <label class="block text-xs font-bold uppercase tracking-wide text-white/50">البلدية</label>
        <DropSelect
    ...
    <div v-if="selectedWilaya" class="space-y-1.5">
      <label class="block text-xs font-bold uppercase tracking-wide text-white/50">طريقة التوصيل</label>
