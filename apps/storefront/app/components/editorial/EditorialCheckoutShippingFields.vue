...
// Editorial's checkout wilaya/commune/shipping picker — same cascade logic as
// CheckoutShippingFields.vue (GET /wilayas + GET /communes?wilayaId=) but
// built on Editorial's own primitives (EditorialSelect) instead of @nuxt/ui,
// matching EditorialLeadFormFields.vue's shipping-method block.
//
...
<template>
  <div class="space-y-4">
    <div class="grid grid-cols-2 gap-4">
      <div>
        <label class="mb-1.5 block text-sm font-semibold text-highlighted">Wilaya</label>
        <EditorialSelect :model-value="props.form.wilayaId" :items="wilayaItems" placeholder="Select wilaya" @update:model-value="(v) => (props.form.wilayaId = v)" />
      </div>
      <div>
        <label class="mb-1.5 block text-sm font-semibold text-highlighted">Commune</label>
        <EditorialSelect v-model="selectedCommuneId" :items="communeItems" :disabled="!selectedWilaya" :placeholder="selectedWilaya ? 'Select commune' : 'Select a wilaya first'" />
      </div>
    </div>

    <div v-if="selectedWilaya" class="space-y-1.5">
      <label class="mb-1.5 block text-sm font-semibold text-highlighted">Shipping method</label>
