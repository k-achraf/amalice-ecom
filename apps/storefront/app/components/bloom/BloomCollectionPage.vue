    ...
    <div class="bg-gradient-to-br from-[var(--color-bloom-hero-from)] to-[var(--color-bloom-hero-to)]">
      <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <nav class="mb-2 flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-neutral-500">
          ...
          <Icon name="i-lucide-chevron-right" class="size-3" />
          <span class="text-[var(--color-bloom-ink)]">{{ props.category.name }}</span>
        </nav>
        <h1 class="font-display text-4xl text-[var(--color-bloom-ink)] sm:text-5xl">{{ props.category.name }}</h1>
        <p v-if="props.category.description" class="mt-3 max-w-2xl text-neutral-600">{{ props.category.description }}</p>
      </div>
    </div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div class="flex gap-8">
          ...
          <div class="glow-card sticky top-24 space-y-6 p-5">
            <div>
              ...
              <div class="flex items-center gap-2">
                <BloomInput :model-value="props.minPrice?.toString() ?? ''" placeholder="Min" @update:model-value="onMinInput" />
                <span class="text-neutral-300">–</span>
                <BloomInput :model-value="props.maxPrice?.toString() ?? ''" placeholder="Max" @update:model-value="onMaxInput" />
              </div>
              <BloomButton block size="sm" class="mt-3" @click="props.onApplyPrice">Apply filter</BloomButton>
            </div>
        ...
        <div class="min-w-0 flex-1">
          <p class="mb-6 text-sm font-medium uppercase tracking-wide text-neutral-400">{{ props.data?.items.length ?? 0 }} products in this collection</p>
          ...
          <EmptyState v-else-if="!props.data?.items.length" icon="i-lucide-search-x" title="No products in this collection yet" description="Check back soon, or browse the full catalog.">
            <BloomButton to="/catalog" class="mt-4">Browse all products</BloomButton>
          </EmptyState>
          <template v-else>
