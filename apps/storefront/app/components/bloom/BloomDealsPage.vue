  ...
  <div>
    <div class="bg-gradient-to-br from-[var(--color-bloom-hero-from)] to-[var(--color-bloom-hero-to)]">
      <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <nav class="mb-6 flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-neutral-500">
          ...
          <span class="text-[var(--color-bloom-ink)]">Deals</span>
        </nav>
        <span class="glow-dot"><Icon name="i-lucide-sparkle" class="size-3.5" /> Limited-time</span>
        <h1 class="font-display mt-5 text-4xl text-[var(--color-bloom-ink)] sm:text-5xl lg:text-6xl">Today's glow deals</h1>
        <p class="mt-3 max-w-xl text-neutral-600">A curated edit of our favorite picks. Cash on delivery — pay when it arrives.</p>

        <div class="mt-8 flex items-center gap-3">
          <span class="text-sm font-medium text-neutral-500">Ends in</span>
          <div class="flex items-center gap-3">
            <div class="flex size-16 flex-col items-center justify-center rounded-full bg-white shadow-[var(--shadow-bloom-sm)]">
    ...
    <section class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
      <div class="mb-8 flex items-end justify-between">
        <div>
          <h2 class="font-display text-3xl text-[var(--color-bloom-ink)]">Flash picks</h2>
          <p class="mt-1 text-sm text-neutral-500">{{ props.data?.items.length ?? 0 }} products on offer</p>
        </div>
        <BloomButton to="/catalog" variant="outline" trailing-icon="i-lucide-arrow-right">Browse all</BloomButton>
      </div>
      ...
      <div v-if="props.data?.items.length" class="grid grid-cols-2 gap-6 lg:grid-cols-4">
        <div v-for="p in props.data.items" :key="p.id" class="space-y-2">
          ...
          <div class="rounded-2xl bg-white px-3 py-2 shadow-[var(--shadow-bloom-sm)]">
