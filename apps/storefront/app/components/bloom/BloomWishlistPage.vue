  ...
  <div>
    <div class="border-b border-neutral-100 bg-[var(--color-bloom-blush)]">
      <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-14 lg:px-8">
        ...
        <h1 class="font-display flex items-center gap-2 text-4xl text-[var(--color-bloom-ink)] sm:text-5xl">
          <Icon name="i-lucide-heart" class="size-8 text-primary-500" /> Saved favorites
        </h1>
    ...
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div v-if="props.savedProducts.length" class="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-4">
        <div v-for="p in props.savedProducts" :key="p.id" class="relative">
          ...
          <button
            class="absolute -right-2 -top-2 z-10 flex size-8 items-center justify-center rounded-full bg-primary-500 text-white shadow-[var(--shadow-bloom-sm)] transition-colors hover:bg-primary-600"
