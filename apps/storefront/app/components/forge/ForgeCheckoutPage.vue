...
// No UForm/UStepper: address validation runs the passed Zod schema directly
// on submit (same gate UForm applied internally), the step indicator is
// plain divs.
              ...
              <div class="border-[3px] border-[var(--color-forge-ink)] bg-white p-6 sm:p-8">
                ...
                <div v-if="props.step === 'address'">
                  ...
                  <form class="space-y-5" @submit.prevent="submitAddress">
                    ...
                    <ForgeCheckoutShippingFields :form="props.form" />
                    <p v-if="props.addressError" class="mt-1 text-xs font-bold text-red-700">{{ props.addressError }}</p>
                ...
                <div v-else-if="props.step === 'review'">
                  ...
                  <div class="mt-4 space-y-1 border-[3px] border-[var(--color-forge-ink)] p-4 text-sm">
                    ...
                    <p class="text-[var(--color-forge-ink)]/70">{{ props.form.address.city }}, {{ props.form.address.region }}</p>
                    <p class="text-[var(--color-forge-ink)]/70">{{ props.form.phone }}</p>
                  </div>

                  <div class="mt-4 flex items-center justify-between border-[3px] border-[var(--color-forge-ink)] p-4 text-sm">
                    <span class="text-[var(--color-forge-ink)]/60">Shipping ({{ props.form.shippingType === 'Home' ? 'Home delivery' : 'Desk delivery' }})</span>
            ...
            <div class="lg:col-span-1">
              <div class="sticky top-24 border-[3px] border-[var(--color-forge-ink)] bg-white p-6">
                ...
                <div class="space-y-2 border-t-[3px] border-[var(--color-forge-ink)] pt-4 text-sm">
                  ...
                  <div class="flex items-center justify-between"><span class="text-[var(--color-forge-ink)]/60">Shipping</span><span v-if="!props.form.shippingType" class="font-bold text-primary-700">Free</span><PriceDisplay v-else :amount-cents="props.form.shippingPriceCents" class="font-bold" /></div>
                </div>
