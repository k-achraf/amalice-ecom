...
// Forge's quantity stepper — plain buttons + number input, no @nuxt/ui.
...
<template>
  <div class="inline-flex items-center divide-x-[3px] divide-[var(--color-forge-ink)] border-[3px] border-[var(--color-forge-ink)]" :class="disabled ? 'opacity-40' : ''">
    <button
      type="button"
    ...
    <input
      ...
      inputmode="numeric"
      :disabled="disabled"
      class="tabular w-12 border-0 bg-white py-2 text-center font-bold focus:outline-none disabled:bg-neutral-100"
      @input="onInput"
    >
    <button
      type="button"
