...
// Forge's lead-form field renderer — same logic as the shared LeadFormFields
// (rows grouping, wilaya→commune cascade backed by GET /wilayas + GET
// /communes?wilayaId=), but built on Forge's own primitives instead of
    ...
    <template v-for="(row, ri) in rows" :key="ri">
      <div :class="row.length === 2 ? 'grid grid-cols-2 gap-3' : ''">
        <div v-for="field in row" :key="field.id" class="space-y-1.5">
          <label class="block text-xs font-bold uppercase tracking-wide text-[var(--color-forge-ink)]/70">{{ field.label }}</label>
          <ForgeSelect
    ...
    <div v-if="selectedWilaya" class="space-y-1.5">
      ...
      <div v-else class="space-y-2">
        <button
          ...
          :class="props.data.shippingType === 'Home' ? 'border-[var(--color-forge-ink)] bg-[var(--color-forge-ink)] text-white' : 'border-[var(--color-forge-ink)]/10 text-[var(--color-forge-ink)] hover:border-[var(--color-forge-ink)]/40'"
        ...
        <button
          ...
          type="button"
          class="flex w-full items-center justify-between border-2 px-4 py-2.5 text-left text-sm transition-colors"
          :class="props.data.shippingType === 'Desk' ? 'border-[var(--color-forge-ink)] bg-[var(--color-forge-ink)] text-white' : 'border-[var(--color-forge-ink)]/10 text-[var(--color-forge-ink)] hover:border-[var(--color-forge-ink)]/40'"
