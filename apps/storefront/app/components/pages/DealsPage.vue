  ...
  <div>
    <div class="relative overflow-hidden bg-gradient-to-br from-primary-600 via-primary-700 to-primary-900">
      ...
      <div class="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
        <nav class="mb-8 flex items-center gap-2 text-sm text-white/50">
          ...
          <span class="text-white/80">العروض</span>
        </nav>
        <h1 class="text-3xl font-bold text-white sm:text-4xl lg:text-5xl">عروض اليوم</h1>
        <p class="mt-3 max-w-xl text-white/70">تشكيلة مختارة من أفضل منتجاتنا. الدفع عند الاستلام.</p>
        <div class="mt-8 flex items-center gap-3">
          <span class="text-sm font-medium text-white/70">ينتهي خلال:</span>
          <div class="flex items-center gap-2">
            <div class="flex size-12 flex-col items-center justify-center rounded-xl bg-white/15 backdrop-blur-sm">
              ...
              <span class="text-[10px] uppercase text-white/60">سا</span>
            </div>
            ...
            <div class="flex size-12 flex-col items-center justify-center rounded-xl bg-white/15 backdrop-blur-sm">
              ...
              <span class="text-[10px] uppercase text-white/60">دق</span>
            </div>
            <span class="text-xl font-bold text-white/60">:</span>
            <div class="flex size-12 flex-col items-center justify-center rounded-xl bg-white/15 backdrop-blur-sm">
              <span class="tabular text-lg font-bold text-white">{{ props.pad(props.timeLeft.seconds) }}</span>
              <span class="text-[10px] uppercase text-white/60">ثا</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div class="mb-6 flex items-end justify-between">
        <div>
          <h2 class="text-2xl font-bold text-highlighted">مختارات سريعة</h2>
          <p class="mt-1 text-sm text-muted">{{ props.data?.items.length ?? 0 }} منتج</p>
        </div>
        <Button to="/catalog" color="neutral" variant="outline" trailing-icon="i-lucide-arrow-left">تصفح الكل</Button>
      </div>
      ...
      <EmptyState v-else icon="i-lucide-package-search" title="لا توجد عروض حالياً" description="تحقق مرة أخرى قريباً لمشاهدة مختاراتنا." />
    </section>
  </div>
</template>
