...
// Promify's lead-form field renderer — same logic as the shared
// LeadFormFields (rows grouping, wilaya→commune cascade), built on
// Promify's own primitives instead of @nuxt/ui.
    ...
    <template v-for="(row, ri) in rows" :key="ri">
      <div :class="row.length === 2 ? 'grid grid-cols-2 gap-3' : ''">
        <div v-for="field in row" :key="field.id" class="space-y-1">
          <label class="block text-xs font-medium text-neutral-600">{{ field.label }}</label>
          <PromifySelect
