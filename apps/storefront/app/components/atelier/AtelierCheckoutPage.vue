...
// Atelier checkout — glow-card form left, sticky glow-card order summary
...
// step indicator is circular dots, OTP uses AtelierPinInput.
    ...
    <div class="velvet-panel">
      <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <nav class="mb-2 flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-[var(--color-atelier-cream)]/50">
          ...
          <span class="text-[var(--color-atelier-cream)]/80">Checkout</span>
        </nav>
        <h1 class="font-display text-4xl text-[var(--color-atelier-cream)] sm:text-5xl">Checkout</h1>
      </div>
    </div>

    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <ClientOnly>
        <AtelierEmptyState v-if="props.cart.items.length === 0" icon="i-lucide-shopping-bag" title="Your bag is empty" description="Add a piece before checking out.">
          ...
          <div class="mb-10 flex items-center justify-center gap-3">
            <template v-for="(s, i) in props.stepItems" :key="s.value">
              <div class="flex items-center gap-2">
                ...
                >{{ i + 1 }}</div>
                <span class="hidden text-sm font-medium uppercase tracking-wide sm:inline" :class="s.value === props.step ? 'text-[var(--color-atelier-ink)]' : 'text-neutral-400'">{{ s.title }}</span>
              </div>
            ...
            <div class="lg:col-span-2">
              <div class="glow-card p-6 sm:p-8">
                ...
                <div v-if="props.step === 'address'">
                  <h2 class="font-display mb-1 text-2xl text-[var(--color-atelier-ink)]">Delivery details</h2>
                  <p class="mb-6 text-sm text-neutral-500">Where should we send your piece?</p>
                  <form class="space-y-5" @submit.prevent="submitAddress">
                    <div>
                      ...
                      <AtelierInput v-model="props.form.phone" type="tel" placeholder="+15551234567" icon="i-lucide-phone" />
                    ...
                    <div>
                      ...
                      <AtelierInput v-model="props.form.name" icon="i-lucide-user" />
                    </div>
                    <div>
                      <label class="mb-1.5 block text-xs font-medium uppercase tracking-wide text-neutral-500">Address line 1</label>
                      <AtelierInput v-model="props.form.address.line1" icon="i-lucide-map-pin" />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <AtelierInput v-model="props.form.address.region" />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <AtelierInput v-model="props.form.address.postalCode" />
                      ...
                      <div>
                        ...
                        <AtelierInput v-model="props.form.address.country" :maxlength="2" class="uppercase" />
                        <p v-if="fieldErrors['address.country']" class="mt-1 text-xs font-medium text-primary-600">{{ fieldErrors['address.country'] }}</p>
                      </div>
                    </div>
                    <AtelierButton type="submit" block size="lg" trailing-icon="i-lucide-arrow-right">Continue to review</AtelierButton>
                  </form>
                </div>

                <!-- Step 2: review -->
                <div v-else-if="props.step === 'review'">
                  <h2 class="font-display mb-1 text-2xl text-[var(--color-atelier-ink)]">Review your order</h2>
                  <p class="mb-6 text-sm text-neutral-500">Everything look right?</p>

                  <div class="space-y-3 rounded-2xl bg-neutral-50 p-4">
                    <h3 class="text-sm font-semibold text-[var(--color-atelier-ink)]">Items</h3>
                  ...
                  <div class="mt-4 space-y-1 rounded-2xl border border-neutral-100 p-4 text-sm">
                    <h3 class="mb-2 font-semibold text-[var(--color-atelier-ink)]">Deliver to</h3>
                    <p class="font-medium text-[var(--color-atelier-ink)]">{{ props.form.name || props.form.phone }}</p>
                  ...
                  <p v-if="props.placeError" class="mt-4 rounded-2xl bg-primary-600 p-3 text-sm font-medium text-white">{{ props.placeError }}</p>

                  <div class="mt-6 flex gap-3">
                    <AtelierButton variant="outline" icon="i-lucide-arrow-left" @click="props.onBack">Back</AtelierButton>
                    <AtelierButton block size="lg" :loading="props.placing" @click="props.onPlaceOrder">Place order</AtelierButton>
                  </div>
                </div>

                <!-- Step 3: OTP -->
                <div v-else-if="props.step === 'otp'">
                  <h2 class="font-display mb-1 text-2xl text-[var(--color-atelier-ink)]">Confirm your order</h2>
                  <p class="mb-6 leading-relaxed text-neutral-600">
                    We sent a 6-digit code to <span class="font-medium text-[var(--color-atelier-ink)]">{{ props.form.phone }}</span>. Enter it below to confirm.
                  </p>
                  <div class="rounded-2xl bg-neutral-50 p-6">
                    <AtelierPinInput v-model="props.otpCode" :length="6" />
                  </div>
                  <p v-if="props.otpError" class="mt-4 rounded-2xl bg-primary-600 p-3 text-sm font-medium text-white">{{ props.otpError }}</p>
                  <AtelierButton block size="lg" :loading="props.verifying" :disabled="props.otpCodeString.length !== 6" class="mt-6" @click="props.onVerifyCode">Confirm order</AtelierButton>
                  <AtelierButton variant="ghost" block class="mt-2" :disabled="props.resendCooldown > 0" @click="props.onResendCode">
              ...
              <div class="glow-card sticky top-24 p-6">
                ...
                <ul class="mb-4 max-h-64 space-y-3 overflow-y-auto pr-1">
                  <li v-for="item in props.cart.items" :key="item.productId" class="flex items-center gap-3">
                    <div class="size-12 shrink-0 overflow-hidden rounded-xl bg-[var(--color-atelier-velvet)]">
                      <NuxtImg v-if="item.imageUrl" :src="item.imageUrl" :alt="item.name" class="size-full object-cover" width="48" height="48" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <p class="line-clamp-1 text-sm font-medium text-[var(--color-atelier-ink)]">{{ item.name }}</p>
                      <p class="text-xs text-neutral-400">Qty {{ item.quantity }}</p>
                    </div>
                    <PriceDisplay :amount-cents="item.priceCents * item.quantity" class="text-sm font-medium text-[var(--color-atelier-ink)]" />
                  </li>
                </ul>
