...
// Atelier's OTP pin input — plain <input> boxes + Tailwind, no @nuxt/ui.
// Circular boxes (the ring motif), auto-advances focus on digit entry,
// steps back on backspace, splits a pasted code across all boxes.
...
<template>
  <div class="flex justify-center gap-2.5" @paste="onPaste">
    <input
      ...
      maxlength="1"
      class="size-12 rounded-full border border-neutral-200 bg-white text-center text-xl font-medium tabular-nums text-[var(--color-atelier-ink)] transition-all duration-200 focus:border-primary-400 focus:shadow-[var(--shadow-atelier-sm)] focus:outline-none sm:size-14"
