...
// Hearth's button — plain <button>/NuxtLink + Tailwind, no @nuxt/ui. Cozy
// rounded-xl corners, a barely-there grounded shadow that deepens slightly
// on hover — the opposite interaction language from Atelier's glow bloom
// and Nova's hard-shadow "press."
//
...
const variantClasses = computed<Record<string, string>>(() => ({
  solid: props.color === 'neutral'
    ? 'bg-[var(--color-hearth-ink)] text-white shadow-[var(--shadow-hearth-sm)] hover:bg-neutral-800 hover:shadow-[var(--shadow-hearth-md)]'
    : 'bg-primary-500 text-white shadow-[var(--shadow-hearth-sm)] hover:bg-primary-600 hover:shadow-[var(--shadow-hearth-md)]',
  outline: 'bg-white text-[var(--color-hearth-ink)] border border-neutral-300 hover:border-primary-400 hover:bg-primary-50',
  ghost: 'bg-transparent text-[var(--color-hearth-ink)] hover:bg-neutral-100'
}))
