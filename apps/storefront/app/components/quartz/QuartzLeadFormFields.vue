...
// Quartz's lead-form field renderer — same logic as the shared
// LeadFormFields (rows grouping, wilaya→commune cascade, per-wilaya
// delivery pricing), built on Quartz's own primitives; zero @nuxt/ui. The
// delivery-method rows (.ship-option) are a deliberate recreation of the
// reference COD order page's "التوصيل الى المنزل / التوصيل الى المكتب"
// radio rows with the price aligned to the end.
...
<template>
  <div class="space-y-4">
    <template v-for="(row, ri) in rows" :key="ri">
      <div :class="row.length === 2 ? 'grid grid-cols-2 gap-3' : ''">
        <div v-for="field in row" :key="field.id" class="space-y-1.5">
          <label class="block text-xs font-semibold text-neutral-600">{{ leadFieldLabel(field) }}</label>
          <QuartzSelect
    ...
    <div v-if="selectedWilaya" class="space-y-1.5">
      <label class="block text-xs font-semibold text-neutral-600">طريقة التوصيل</label>
      <div v-if="!selectedWilaya.homeDeliveryEnabled && !selectedWilaya.deskDeliveryEnabled" class="rounded-xl border border-neutral-200 bg-neutral-50 p-3 text-sm text-neutral-500">
      ...
      <div v-else class="space-y-2">
        <button
          ...
          type="button"
          class="ship-option"
          :class="{ selected: props.data.shippingType === 'Home' }"
          @click="selectShipping('Home', selectedWilaya.homeDeliveryPriceCents ?? 0)"
        >
          <span class="flex items-center gap-2 text-sm font-semibold text-neutral-900"><Icon name="i-lucide-home" class="size-4 text-primary-700" /> التوصيل إلى المنزل</span>
          <PriceDisplay :amount-cents="selectedWilaya.homeDeliveryPriceCents ?? 0" class="text-sm font-semibold text-neutral-900" />
        </button>
        <button
          v-if="selectedWilaya.deskDeliveryEnabled"
          type="button"
          class="ship-option"
          :class="{ selected: props.data.shippingType === 'Desk' }"
          @click="selectShipping('Desk', selectedWilaya.deskDeliveryPriceCents ?? 0)"
        >
          <span class="flex items-center gap-2 text-sm font-semibold text-neutral-900"><Icon name="i-lucide-building-2" class="size-4 text-primary-700" /> الاستلام من مكتب التوصيل</span>
          <PriceDisplay :amount-cents="selectedWilaya.deskDeliveryPriceCents ?? 0" class="text-sm font-semibold text-neutral-900" />
        </button>
      </div>
    </div>
  </div>
