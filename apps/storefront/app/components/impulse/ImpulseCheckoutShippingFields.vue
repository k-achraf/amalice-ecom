...
// Impulse checkout's wilaya/commune/shipping picker — same cascade logic as
// CheckoutShippingFields.vue (GET /wilayas + GET /communes?wilayaId=), built
// on Impulse's own big-touch-target primitives (ImpulseSelect) instead of
// @nuxt/ui's Select, matching ImpulseLeadFormFields.vue's visual language.
//
  ...
  <div class="space-y-4">
    <div class="grid grid-cols-2 gap-3">
      <div class="space-y-1.5">
        <label class="block text-xs font-bold uppercase tracking-wide text-neutral-600">الولاية</label>
        <ImpulseSelect :model-value="props.form.wilayaId" :items="wilayaItems" placeholder="اختر الولاية" @update:model-value="(v) => (props.form.wilayaId = v)" />
      </div>
      <div class="space-y-1.5">
        <label class="block text-xs font-bold uppercase tracking-wide text-neutral-600">البلدية</label>
        <ImpulseSelect v-model="selectedCommuneId" :items="communeItems" :disabled="!selectedWilaya" :placeholder="selectedWilaya ? 'اختر البلدية' : 'اختر الولاية أولاً'" />
      </div>
    </div>

    <div v-if="selectedWilaya" class="space-y-1.5">
      <label class="block text-xs font-bold uppercase tracking-wide text-neutral-600">طريقة التوصيل</label>
      ...
      <div v-else class="space-y-2">
        <button
          ...
          type="button"
          class="flex w-full items-center justify-between border-2 px-4 py-3 text-start text-sm transition-colors"
        ...
        >
          <span class="flex items-center gap-2 font-bold"><Icon name="i-lucide-home" class="size-4" /> التوصيل إلى المنزل</span>
          <PriceDisplay :amount-cents="selectedWilaya.homeDeliveryPriceCents ?? 0" class="font-bold" />
        </button>
        <button
        ...
        >
          <span class="flex items-center gap-2 font-bold"><Icon name="i-lucide-building-2" class="size-4" /> الاستلام من مكتب التوصيل</span>
          <PriceDisplay :amount-cents="selectedWilaya.deskDeliveryPriceCents ?? 0" class="font-bold" />
        </button>
      </div>
    </div>
  </div>
