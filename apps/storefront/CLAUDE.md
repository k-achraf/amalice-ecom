...
- **Pages/sections**: `app/components/TemplatePage.vue` and `TemplateSection.vue` resolve `(pageName/sectionName, activeTemplate)` → the right presentational component, falling back to the bare `minimal` version. Every override is `defineAsyncComponent` (dynamic `import()`) — **not** wrapped in `<Suspense>`, deliberately:
  ...
  - `<Suspense>` was tried around this, to fix a PageSpeed "Hydration Mismatch" diagnostic, and reverted after it caused a measured, severe CLS regression (0.001 → 1.000, the max possible) in production: resolving the chunk client-side means a real network fetch (no benefit from Node's warm SSR module cache), and while pending, Suspense renders nothing for the whole page-content subtree — so the entire page pops in at once when it resolves, and the LCP element (which lives inside this subtree) doesn't exist in the DOM at all until then either. **Do not re-add `<Suspense>` here** without a way to avoid blanking the already-SSR-rendered DOM while the client-side chunk loads — a CLS=1.0 regression on a scored Core Web Vital is far worse than an unscored hydration-mismatch diagnostic.
