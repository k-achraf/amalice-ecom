...
const MAX_URL_DOWNLOAD = 10 * 1024 * 1024 // 10 MB
...
export class UploadController {
  ...
  async uploadFromUrl(@Body() body: { url: string }) {
    ...
    if (!contentType.startsWith('image/')) {
      throw new BadRequestException(`URL is not an image (content-type: ${contentType})`)
    }

    const buffer = Buffer.from(await response.arrayBuffer())
    if (buffer.length > MAX_URL_DOWNLOAD) {
      throw new BadRequestException(`Image too large (${(buffer.length / 1024 / 1024).toFixed(1)} MB, max 10 MB)`)
    }
